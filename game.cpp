...
void runGame()
{
  ...
  while (window.isOpen())
  {
    ...
    if (sf::Event::KeyPressed)
    {
      ...
      {
        double cosAlpha=cos(player.Angle + pi / 8);
        double sinAlpha=sin(player.Angle + pi / 8);

        if (World[(int)(player.X + player.StepSize * cosAlpha)][(int)player.Y] == 1)
          player.X += 0;
        else
          player.X += player.StepSize * cosAlpha;
        if (World[(int)player.X][(int)(player.Y + player.StepSize * sinAlpha)] == 1)
          player.Y += 0;
        else
          player.Y += player.StepSize * sinAlpha;
      ...
      if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left)) player.Angle -= 0.03;

      if (sf::Keyboard::isKeyPressed(sf::Keyboard::Right)) player.Angle += 0.03;
